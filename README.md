...
- Результаты: 
  ...
  в связи счем снижается время на задачу в среднем
      Результаты приведены ниже:
       ...
       | queueSize                | 2      | 10     | 20   | 30   | 50    |
       |--------------------------|--------|--------|------|------|-------|
       | Затраченное время:       | 10     | 11     | 10   | 10   | 9     |
       | Пропущенные задачи       | 172    | 108    | 28   | 0    | 0     |
       | Выполненые задачи        | 28     | 92     | 172  | 200  | 200   |
       | Среднее время на задачу: | 0,3571 | 0,1195 | 0,58 | 0,05 | 0,045 |
...
2. **Максимальное количество потоков (`maxPoolSize`)**
   ...
   - Результаты приведены ниже:
        ...
        | maxPoolSize              | 6     | 7     | 8     |
        |--------------------------|-------|-------|-------|
        | Затраченное время:       | 10    | 11    | 11    | 
        | Пропущенные задачи       | 42    | 15    | 15    | 
        | Выполненые задачи        | 158   | 185   | 185   |
        | Среднее время на задачу: | 0,063 | 0,059 | 0,059 | 
...
не влияют на  работку самого пула, все уперается в `maxPoolSize` и зависит от этого параметра. 
   - Результаты приведены ниже:
     ...
     | corePoolSize             | 2      | 3     | 4    |
     |--------------------------|--------|-------|------|
     | Затраченное время:       | 11     | 11    | 10   | 
     | Пропущенные задачи       | 0      | 0     | 0    | 
     | Выполненые задачи        | 200    | 200   | 200  |
     | Среднее время на задачу: | 0,055  | 0,055 | 0,05 | 
